...
import { QUERY_FILES, MUTATION_CREATE_UPLOAD, MUTATION_ABORT_UPLOAD, MUTATION_COMPLETE_UPLOAD } from '../entities/Queries'
...
export async function createUpload(navAbilityClient: NavAbilityClient, file:FileInput, parts: Number ):Promise<UploadInfo> {
...
export async function completeUpload(navAbilityClient: NavAbilityClient, fileId: string, completedUpload:CompletedUploadInput) {
    await navAbilityClient.mutate({
